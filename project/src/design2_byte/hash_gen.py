...
F = finite_field_constructor.GF(misc1.NUM_SIZE,
                                modulus=(1, 0, 0, 0, 1, 1, 0, 1, 1),
                                name='x')
...
class State(object):
    ...
    def __init__(self, input):
        ...
        """
        t = len(input)
        if t != misc1.LANE_LENGTH:
            raise TypeError(f'l is {t} long rather than {misc1.LANE_LENGTH}!')
        t = len(input[0])
        if t != misc1.COLUMN_LENGTH:
            raise TypeError(
                f'l[0] is {t} long rather than {misc1.COLUMN_LENGTH}!')
        t = len(input[0][0])
        if t != misc1.ROW_LENGTH:
            raise TypeError(
                f'l[0][0] is {t} long rather than {misc1.ROW_LENGTH}!')
        self.__storage = [matrix(ll) for ll in input]

    def xor_with_message(self, message, use_fore):
        zstart = 0 if use_fore else misc1.LANE_LENGTH // 2
        zend = misc1.LANE_LENGTH // 2 if use_fore else misc1.LANE_LENGTH
        m_i = 0
        for z in range(zstart, zend):
            for y in range(0, misc1.COLUMN_LENGTH):
                for x in range(0, misc1.ROW_LENGTH):
    ...
    def map(self, f):
        for z in range(misc1.LANE_LENGTH):
            self[z] = f(self[z])

    def lane(self, y, x):
        return tuple(self[z, y, x] for z in range(misc1.LANE_LENGTH))

    def set_lane(self, y, x, t):
        if len(t) != misc1.LANE_LENGTH:
            raise ValueError(
                f'lane should be {misc1.LANE_LENGTH} long but got {len(t)}!')
        for z in range(misc1.LANE_LENGTH):
            self[z, y, x] = t[z]
    ...
    def __repr__(self):
        return str(self.turncate_to_first_n_slice(misc1.LANE_LENGTH))
...
class HashFunction(object):
    ...
    @staticmethod
    def fill_and_block_message(message):
        mds = list(message)
        parts = list(mds[i:i + misc1.BLOCK_SIZE]
                     for i in range(0, len(mds), misc1.BLOCK_SIZE))
        while len(parts[-1]) < misc1.BLOCK_SIZE:
            parts[-1].append(0)
        return tuple(
    ...
    def tuple_to_mds(t):
        return matrix(
            tuple(tuple(map(lambda x: F.fetch_int(int(x)), l)) for l in t))

    def __init__(self, round, round_constant, mds, shift_column_constants,
                 lane_transform_constants, sbox):
        self.__ROUND = round
        ...
        self.__INIT_STATE = State([[[102, 89, 200, 212], [46, 130, 96, 85],
                                    [26, 100, 115, 79], [134, 216, 139, 231]],
                                   [[43, 214, 106, 179], [92, 98, 67, 126],
                                    [232, 31, 204, 186], [229, 42, 14, 255]],
                                   [[182, 29, 38, 254], [132, 147, 133, 240],
                                    [135, 105, 101, 146], [3, 168, 172, 184]],
                                   [[151, 9, 234, 81], [137, 114, 167, 238],
                                    [162, 174, 187, 220], [236, 201, 80, 193]],
                                   [[37, 175, 62, 195], [219, 178, 112, 145],
                                    [59, 180, 150, 36], [140, 158, 107, 190]],
                                   [[117, 203, 192, 153], [39, 169, 34, 185],
                                    [152, 64, 171, 223], [241, 118, 165, 149]],
                                   [[141, 73, 252, 199], [242, 53, 19, 208],
                                    [191, 48, 196, 161], [233, 108, 77, 25]],
                                   [[213, 15, 123, 131], [127, 156, 88, 142],
                                    [173, 22, 75, 166], [58, 144, 181, 183]]])
    ...
    def __call__(self, input_):
        ...
        return self.block_to_hash(
            state.turncate_to_first_n_slice(misc1.FINAL_USE_SLICE))
...
'''
    ...
    def __substitute_bytes(self, s, _):
        s.map(lambda m: m.apply_map(lambda x: self.sbox[x // SBox.BASE, x %
                                                        SBox.BASE]))
        return s
    ...
    def __shift_column(self, s, _):
        for z in range(misc1.LANE_LENGTH):
            slice = s[z]
            for x in range(misc1.ROW_LENGTH):
    ...
    def __lane_transform(self, s, _):
        for x in range(misc1.ROW_LENGTH):
            for y in range(misc1.COLUMN_LENGTH):
...
def generate_hash_function(round, round_constant, mds, shift_column_constants,
                           lane_transform_constants, sbox):
    global f
    f = HashFunction(round, round_constant, mds, shift_column_constants,
                     lane_transform_constants, sbox)
...
if __name__ == '__main__':
    lane_transform_constants = ((2, 7, 2, 3), (0, 4, 3, 6), (4, 3, 1, 4),
                                (1, 1, 5, 0))
    generate_hash_function(14, 247, misc1.mdses[6][0], (3, 2, 1, 2),
                           lane_transform_constants, misc1.sboxes[0])
    test_cases = (
        tuple(bytes(16)),
        (0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a,
         0x0b, 0x0c, 0x0d, 0x0e, 0x0f),
        tuple(bytes(4)),
        (0xd3, 0x60, 0xfc, 0x16, 0x01, 0x75, 0x55, 0x39, 0xf2, 0x64, 0xdf,
         0x53, 0x65, 0x59, 0xb8, 0x56),
    )
    for test_case in test_cases:
        print('Test Case:', test_case)
        print('Result:', hash_(test_case))
