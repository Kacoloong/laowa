...
class FunctionModel:
    ...
    def R(self, fNa, in_var_v_m, in_var_d_m, in_var_v_a0, in_var_d_a0, in_var_v_a1, in_var_d_a1,
        ...
        if self.__bounds_rounds - 4 <= step:
            self.expand_model(in_var_v_b3, in_var_d_b3, in_var_v_c2, in_var_d_c2,
                              self.right_shift(in_var_v_a4, rotation_Constant_right[step]),
                              self.right_shift(in_var_d_a4, rotation_Constant_right[step]), 1)
        else:
            self.expand_model(in_var_v_b3, in_var_d_b3, in_var_v_c2, in_var_d_c2,
                              self.right_shift(in_var_v_a4, rotation_Constant_right[step]),
                              self.right_shift(in_var_d_a4, rotation_Constant_right[step]), 0)
    ...
    def assign_value(self):
        ...
        for i in range(11, 32):
            for j in range(32):
                if i == 31 and j == 31:
                    obj += "0bin000000000@xd_%s_%s) = 0bin" % (i, j) + bin(16)[2:].zfill(10) + ";\n"
                else:
        ...
        for i in range(4, 8):
            for j in range(32):
                if i == 7 and j == 31:
                    obj += "0bin000000000@xd_%s_%s) = 0bin" % (i, j) + bin(20)[2:].zfill(10) + ";\n"
                else:
                    obj += "0bin000000000@xd_%s_%s" % (i, j) + ", "
        self.__constraints.append(obj)
        obj = "ASSERT BVPLUS(10,"
        for i in range(4, 6):
            for j in range(32):
                if i == 5 and j == 31:
                    obj += "0bin000000000@xd_%s_%s) = 0bin" % (i, j) + bin(5)[2:].zfill(10) + ";\n"
                else:
                    obj += "0bin000000000@xd_%s_%s" % (i, j) + ", "
        self.__constraints.append(obj)
    ...
    def main(self):
        print(f"start step:{self.__step - 4}")
        print(f"end step:{self.__bounds_rounds - 4}")
        in_var_v_a = []
        in_var_d_a = []
        in_var_v_m = []
        in_var_d_m = []
        ...
        for i in range(self.__step, self.__bounds_rounds):
            self.R(Bool_right[(i - 4) // 16],
                   in_var_v_m[message_order_right[(i)]],
                   in_var_d_m[message_order_right[(i)]],
                   in_var_v_a[i - 4],
                   in_var_d_a[i - 4],
                   in_var_v_a[i - 3],
                   in_var_d_a[i - 3],
    ...
    def hanele_data(self, data_list):
    ...
    def solver(self):
        ...
        for i in range(90, -1, -1):
            ...
            if "Valid.\n" != R.decode():
                self.hanele_data(R.decode())
                file = open("right_res_" + str(i) + ".out", "w")
