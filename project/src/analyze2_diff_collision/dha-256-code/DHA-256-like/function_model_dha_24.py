...
class FunctionModel:
    def __init__(self, steps, bounds, message_bound, msg_rot, state_rot):

        self.__bounds_rounds = bounds
        self.__message_bound = message_bound
        self.__step = steps
        self.__msg_rot = msg_rot
        self.__state_rot = state_rot
        self.__declare = []  # 存储变量
    ...
    def R(self, in_var_v_m, in_var_d_m, in_var_v_a0, in_var_d_a0, in_var_v_a1, in_var_d_a1, in_var_v_a2, in_var_d_a2,
        ...
        self.xor_function(in_var_v_e3, in_var_d_e3,
                          self.left_shift(in_var_v_e3, self.__state_rot[0]),
                          self.left_shift(in_var_d_e3, self.__state_rot[0]),
                          self.left_shift(in_var_v_e3, self.__state_rot[1]),
                          self.left_shift(in_var_d_e3, self.__state_rot[1]),
                          in_var_v_b[0], in_var_d_b[0],
                          in_var_e3,
                          self.left_shift(in_var_e3, self.__state_rot[0]),
                          self.left_shift(in_var_e3, self.__state_rot[1]))
        ...
        self.xor_function(in_var_v_a3, in_var_d_a3,
                          self.left_shift(in_var_v_a3, self.__state_rot[2]),
                          self.left_shift(in_var_d_a3, self.__state_rot[2]),
                          self.left_shift(in_var_v_a3, self.__state_rot[3]),
                          self.left_shift(in_var_d_a3, self.__state_rot[3]),
                          in_var_v_b[5], in_var_d_b[5],
                          in_var_a3,
                          self.left_shift(in_var_a3, self.__state_rot[2]),
                          self.left_shift(in_var_a3, self.__state_rot[3]))
    ...
    def message_expand(self, in_var_v_w0, in_var_d_w0, in_var_v_w1, in_var_d_w1, in_var_v_w2, in_var_d_w2, in_var_v_w3,
        ...
        self.xor_function(in_var_v_w0, in_var_d_w0,
                          self.left_shift(in_var_v_w0, self.__msg_rot[0]),
                          self.left_shift(in_var_d_w0, self.__msg_rot[0]),
                          self.left_shift(in_var_v_w0, self.__msg_rot[1]),
                          self.left_shift(in_var_d_w0, self.__msg_rot[1]),
                          in_var_v_b0, in_var_d_b0,
                          in_var_w0,
                          self.left_shift(in_var_w0, self.__msg_rot[0]),
                          self.left_shift(in_var_w0, self.__msg_rot[1]))
        ...
        self.xor_function(in_var_v_w2, in_var_d_w2,
                          self.left_shift(in_var_v_w2, self.__msg_rot[2]),
                          self.left_shift(in_var_d_w2, self.__msg_rot[2]),
                          self.left_shift(in_var_v_w2, self.__msg_rot[3]),
                          self.left_shift(in_var_d_w2, self.__msg_rot[3]),
                          in_var_v_b2, in_var_d_b2,
                          in_var_w2,
                          self.left_shift(in_var_w2, self.__msg_rot[2]),
                          self.left_shift(in_var_w2, self.__msg_rot[3]))
    ...
    def assign_value(self):
        ...
        for i in range(15, 17):
            for j in range(32):
                ...
                temp += "ASSERT %s = 0bin0;\n" % (self.save_variable("wd_" + str(i) + "_" + str(31 - j)))
                self.__constraints.append(temp)
        for i in range(18, self.__message_bound):
            for j in range(32):
                temp = "ASSERT %s = 0bin0;\n" % (self.save_variable("wv_" + str(i) + "_" + str(31 - j)))
                temp += "ASSERT %s = 0bin0;\n" % (self.save_variable("wd_" + str(i) + "_" + str(31 - j)))
                self.__constraints.append(temp)
...
if __name__ == '__main__':
    ...
    bounds = 18
    message_bound = 24
    msg_data = input("please input msg rotate () :").split(",")
    msg_rot = [0, 0, 0, 0]
    state_rot = [0, 0, 0, 0]
    msg_rot[0] = int(msg_data[0])
    msg_rot[1] = int(msg_data[1])
    msg_rot[2] = int(msg_data[2])
    msg_rot[3] = int(msg_data[3])

    state_data = input("please input state rotate () :").split(",")

    state_rot[0] = int(state_data[0])
    state_rot[1] = int(state_data[1])
    state_rot[2] = int(state_data[2])
    state_rot[3] = int(state_data[3])

    FunctionModel(step, bounds, message_bound, msg_rot, state_rot).solver()
