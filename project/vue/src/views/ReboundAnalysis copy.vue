                        ...
                        <el-row :gutter="20">
                            <el-col :span="8">
                                <el-form-item>
                                    ...
                                    <el-input v-model="cipher_name" placeholder="AES" type="textarea"
...
export default {
    ...
    const submitForm = async () => {
        const response = await fetch('http://127.0.0.1:5000/api/hash_function/analyze/single-rebound', {
            method: 'POST',
            headers: {
        ...
        if (data.result.success === true) {
            pdfSrc.value = `http://127.0.0.1:5000/api/hash_function/rebound/pdf?path=${encodeURIComponent(data.result.analysis_result.pdf_file)}`;
    ...
    const startAnalysis = async () => {
        ...
        const startResponse = await fetch('http://127.0.0.1:5000/api/hash_function/analyze/multiple-rebound', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    const pollTaskStatus = (taskId) => {
        ...
        pollInterval = setInterval(async () => {
            const statusResponse = await fetch(`http://127.0.0.1:5000/api/hash_function/analyze/rebound/check-status?taskId=${taskId}`, {
            ...
            if (newPdfResult) {
                pdfSrc.value = `http://127.0.0.1:5000/api/hash_function/rebound/pdf?path=${encodeURIComponent(newPdfResult.result.analysis_result.pdf_file)}`;
